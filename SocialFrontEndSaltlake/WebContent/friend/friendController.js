app.controller("friendController", function($scope, $http, $location,$rootScope) 
	{
		...
		$scope.approve=function(friendId)
		{
			...
			$http.get("http://localhost:8090/SocialNetworkAppRest/approvalFriendRequest/"+friendId)
		...
		$scope.reject=function(friendId)
		{
			...
			$http.get("http://localhost:8090/SocialNetworkAppRest/rejectFriendRequest/"+friendId)
			.success(function(response){
		...
		$scope.addFriend=function()
		{
			...
			$http.post("http://localhost:8090/SocialNetworkAppRest/createFriendRequest",$scope.friend)
			.then(function(response){
